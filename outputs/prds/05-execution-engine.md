...
**Implemented:** Sequential and hierarchical crew execution. Flow execution with all step types: crew, function, router (Python function dispatch), condition (safe expression eval), and transform (WASM data massaging). Train and test modes via CrewAI native. Budget enforcement via LiteLLM virtual keys. Guardrails (function, LLM, schema, PII). AgentPolicy enforcement (tool allowlist/denylist, delegation, sandbox tier, PII redaction). Workflow hooks (before/after kickoff, before/after task, before/after flow step, on_error). HITL respond endpoint. SSE + WebSocket streaming. Webhooks. Automation triggers (cron, webhook, API). gRPC API on :50051. AI Copilot (prompt-to-crew). Live collaboration (WebSocket rooms). Full sandbox tier hierarchy: none < wasm < docker/podman < gVisor < MicroVM (stub).

**Deferred to post-MVP:** Temporal workflow backend, MicroVM full implementation (Firecracker/Cloud Hypervisor kernel+rootfs), dynamic task creation in hierarchical mode, warm container/VM pools.
