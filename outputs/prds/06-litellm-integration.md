...
| Scope | Budget Duration | Reset Behavior |
|-------|----------------|----------------|
| **Per-execution** | Matches execution lifetime | No reset — key deleted when execution completes |
| **Per-agent** | Configurable (`budget_duration`, default: `1d`) | LiteLLM resets spend counter at the end of each period. Agent can spend up to `max_budget` per period across all executions. |
| **Per-crew** | Matches crew execution lifetime | No reset — key deleted when crew completes |
| **Per-user** | Configurable (`budget_duration`, default: `30d`) | Monthly budget per human user. Shared across all agent runs the user kicks off. |
