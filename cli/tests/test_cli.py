...
def test_access_token_expiry_reads_exp_claim():
    ...
    from blackbeard_cli.credentials import _REFRESH_MARGIN_S, access_token_expiry

    exp = time_mod.time() + 900
    token = _make_jwt({"sub": "u1", "exp": exp})
    assert access_token_expiry(token) == pytest.approx(exp - _REFRESH_MARGIN_S)
...
def test_access_token_expiry_shorter_than_default_lifetime():
    ...
    expiry = access_token_expiry(token)
...
def test_access_token_expiry_malformed_token_falls_back():
    ...
    from blackbeard_cli.credentials import ACCESS_TOKEN_LIFETIME_S, access_token_expiry

    now = time_mod.time()
    for bad in ["", "not-a-jwt", "a.b.c", "a.!.c", _make_jwt({"sub": "u1"})]:
        expiry = access_token_expiry(bad)
