"""Data retention: periodic purge of aged audit logs and execution records.

Audit logs carry actor identity and (anonymized) IP addresses; executions
carry user-supplied inputs and crew outputs. Both grow unbounded without a
retention limit (GDPR Art. 5(1)(e) storage limitation). Retention is opt-in:
set AUDIT_LOG_RETENTION_DAYS and/or EXECUTION_RETENTION_DAYS to enable.
"""
...
async def purge_expired_data(session: AsyncSession) -> dict[str, int]:
    ...
    purged = {"audit_logs": 0, "executions": 0}
    ...
    if purged["audit_logs"] or purged["executions"]:
        logger.info(
            "Retention purge: %d audit log(s), %d execution(s) deleted",
            purged["audit_logs"],
            purged["executions"],
            extra={
