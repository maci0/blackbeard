...
export default memo(function SwitchNode({ data, selected }: NodeProps) {
  ...
  return (
    <div
      ...
      className={cn(
        'w-[180px] overflow-hidden rounded-lg border bg-card shadow-sm transition-all duration-150',
        selected
    ...
    >
      ...
      <div className="flex items-center gap-1.5 bg-gradient-to-r from-cyan-600 to-cyan-400 px-2 py-1.5">
        <ToggleLeft className="h-3.5 w-3.5 text-white/90" />
        <span className="text-[10px] font-bold uppercase tracking-wider text-white/90">Switch</span>
      </div>
